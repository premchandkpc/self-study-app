...
function buildTwoSumSteps({ nums: numsIn = NUMS, target: tIn = TARGET } = {}) {
  const nums = Array.isArray(numsIn) ? numsIn.filter((v) => Number.isFinite(v)).slice(0, 8) : NUMS;
  if (!nums.length) nums.push(...NUMS);
  const target = Number(tIn) || TARGET;
  const steps = [];
  ...
  const state = {
    ...
    activeIndex: -1,
    resultIndices: [],
    vars: { i: 0, num: nums[0], target, complement: target - nums[0], map: {} },
  ...
  for (let i = 0; i < nums.length; i++) {
    ...
    state.activeIndex = i;
    state.vars = { i, num, target, complement, map: { ...map } };
  ...
  if (!state.resultIndices.length) {
    state.vars = { i: nums.length, num: -1, target, complement: -1, map: { ...map } };
...
export default {
  ...
  inputs: [
    { key: 'nums',   label: 'Numbers (comma-sep)', type: 'array-num', default: NUMS   },
    { key: 'target', label: 'Target sum',          type: 'number',    default: TARGET },
  ],
  code: TWO_SUM_CODE,
  language: 'JavaScript',
  metrics: [
