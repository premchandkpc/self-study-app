...
export default {
  ...
  topicContent: {
    concept: [
      { title: 'DNS Resolution — how Route53 maps names to IPs', content: 'Route53 is AWS\'s managed DNS service that translates human-readable domain names like example.com into IP addresses. It operates through hosted zones, which are DNS database containers for a single domain. Public hosted zones answer queries from the internet while private hosted zones only resolve within your VPC for internal service discovery. Each hosted zone gets four authoritative nameserver records automatically for redundancy across AWS edge locations.' },
      { title: 'Routing Policies — controlling how traffic flows', content: 'Route53 offers eight routing policies: Simple (single record), Weighted (percentage-based traffic split), Latency (lowest-latency region), Geolocation (by user location), Geoproximity (by distance with bias), Failover (active-passive DR), Multi-Value (random from healthy records), and IP-based (by CIDR block). Weighted routing enables canary deployments where 10% of traffic goes to a new version. Latency routing uses AWS\'s global latency map to route users to the fastest region based on measured DNS resolver latency. Geolocation determines the user\'s approximate continent or country from the DNS resolver\'s IP and routes accordingly.' },
      { title: 'Deep — DNS internals, ALIAS vs CNAME, and TTL mechanics', content: 'When a client queries Route53, the DNS resolution chain starts at the browser\'s cache, then the OS resolver, then the ISP\'s recursive resolver which walks the DNS hierarchy: root servers to .com TLD servers to Route53 authoritative nameservers. Route53 responds with the requested record, and each level caches the response according to the TTL value. ALIAS records are unique to Route53 — they work at the zone apex (example.com without www) and Route53 resolves them internally by looking up the target AWS resource\'s A record and returning the IPs directly, with no additional charge. CNAME records only work for subdomains and incur standard query charges. TTL determines how long resolvers cache the response — short TTLs (60s) enable fast failover but increase query costs, while long TTLs (86400s) reduce costs but delay propagation by up to 24 hours.' },
    ],
    why: [
      'DNS is the first interaction point for every user — if DNS is misconfigured, no amount of backend optimization matters because users cannot reach you at all. Route53 is the most feature-rich DNS service on AWS with health-check-based failover, latency routing, and private zone support that collectively provide the foundation for global application delivery and disaster recovery architectures.',
      'Route53 routing policies directly impact application availability, performance, and operational safety. Weighted routing enables canary deployments with immediate rollback by shifting traffic percentages between versions without redeploying infrastructure. Latency routing automatically directs users to the fastest AWS region, improving page load times by 30-50% for global audiences compared to single-region deployments.',
      'TTL management is a critical operational decision balancing cost against failover speed. Short TTLs (60s) allow DNS changes to propagate within minutes but increase query costs proportionally. Long TTLs (86400s) minimize costs but mean DNS failures or migrations take a full day to fully propagate. Enterprise architectures typically use TTL=60s for critical ALIAS records and longer TTLs for static records.',
    ],
    interview: [
      { q: 'What is the difference between an ALIAS record and a CNAME record in Route53?', a: 'ALIAS records work at the zone apex (root domain like example.com without www) and are free of charge, while CNAME records only function for subdomains like www.example.com. ALIAS can point directly to AWS resources such as ALB, CloudFront distributions, S3 website endpoints, and Elastic Beanstalk environments. When a client queries an ALIAS record, Route53 resolves it internally by looking up the target AWS resource\'s A record and returning the IP addresses directly — there is no additional Route53 query charge for the ALIAS resolution. CNAME records cannot coexist with other records at the same name and cannot be used at the zone apex per DNS standards, which is why ALIAS is essential for mapping apex domains to AWS resources.', followUps: ['Can a CNAME point to an apex domain?', 'What happens to ALIAS records during a Route53 health check failure?'] },
      { q: 'How does Route53 failover work and what is the typical RTO?', a: 'Route53 health checks monitor endpoints by sending HTTP or HTTPS requests every 30 seconds by default, or every 10 seconds with fast health checks at additional cost. After N consecutive failures (default 3, approximately 90 seconds), the health check marks the endpoint as unhealthy. With failover routing policy configured, Route53 stops returning the primary record in DNS responses and starts returning the secondary or DR record instead. The RTO depends on the health check interval multiplied by the failure threshold plus the TTL of the DNS record. Typical RTO is 2-3 minutes with standard settings. For sub-60 second RTO, use fast health checks with a 10-second interval and TTL of 20 seconds, though this costs more.', followUps: ['How can you reduce RTO below 60 seconds?', 'What is active-active versus active-passive failover in Route53?'] },
      { q: 'What is Route53 Resolver and how does it enable hybrid DNS architectures?', a: 'Route53 Resolver bridges on-premise DNS and AWS VPC DNS resolution through inbound and outbound endpoints deployed across multiple Availability Zones for high availability. Inbound endpoints allow on-premise DNS servers to forward queries to Route53, enabling resolution of internal AWS hostnames like ip-10-0-1-5.ec2.internal without running custom DNS forwarders on EC2 instances. Outbound endpoints with conditional forwarding rules allow VPC resources to resolve on-premise hostnames by forwarding matching queries to on-premise DNS servers over Direct Connect or VPN. This eliminates the need to manage BIND or Unbound servers and enables hybrid cloud architectures where applications in both environments discover each other by private DNS name.', followUps: ['What is the difference between inbound and outbound Resolver endpoints?', 'How does Route53 Resolver work with Direct Connect versus VPN connections?'] },
    ],
    gotcha: [
      'TTL is king for failover speed — a long TTL of 3600 seconds means DNS resolvers cache old records for an hour even after failover triggers, leaving users hitting a dead endpoint while the secondary is healthy. Always pair health-checked records with short TTLs for acceptable recovery times.',
      'Route53 health checks only monitor from AWS edge locations — they cannot detect client-side network issues or ISP-specific DNS caching problems. An AWS health check may see a 200 OK response while a regional ISP outage prevents real users from reaching your application, and no failover is triggered.',
      'ALIAS records pointing to an ALB in an impaired but not fully failed state may continue resolving to that ALB because Route53 separates DNS record resolution from target health evaluation. Always combine ALIAS records with health checks and failover routing policies for reliable multi-region architectures.',
      'Private hosted zones require both enableDnsSupport and enableDnsHostnames enabled on the VPC — if either setting is off, private zone records will not resolve. Overlapping records between public and private zones for the same domain can cause unpredictable resolution in hybrid environments where resolvers access both zones.',
    ],
    tradeoffs: [
      { pro: 'Route53 is a global service with no regional constraints, providing a single pane of glass for DNS management across all AWS regions. It supports eight routing policies for sophisticated traffic management without deploying infrastructure in every region and integrates deeply with AWS services like ALB, CloudFront, and health checks.', con: 'DNS caching at ISPs and recursive resolvers means DNS changes propagate slowly regardless of Route53 configuration. Even with a 60-second TTL, some ISP resolvers ignore TTL values and cache records for minutes or hours, making fast DNS-based failover unreliable in practice.' },
      { pro: 'Rich routing policy options including weighted, latency, geolocation, geoproximity, failover, and multi-value answer enable sophisticated traffic management like canary deployments, regional content delivery, and global load balancing from a single DNS provider.', con: 'Complex routing configurations often require Route53 Traffic Flow at $50 per month per policy record and can be difficult to debug. Misconfigured routing policies can cause traffic black holes that require deep DNS resolution knowledge to troubleshoot.' },
      { pro: 'Deep integration with health checks, CloudWatch alarms, and AWS services enables automated failover and traffic management without custom scripting, third-party monitoring tools, or separate health check infrastructure.', con: 'Health check costs add up significantly at scale — each standard health check endpoint costs $0.75 per month, and calculated health checks cost $1.50 per month each. For large deployments monitoring hundreds of endpoints, health check costs can easily exceed DNS query costs by a factor of 10.' },
    ],
  },
};
