...
function buildEC2Steps() {
  const steps = []; const s = {
    nodes: [
      svc('dev',      'Developer',          'client',  30,  190),
      svc('ec2',      'EC2 Instance',       'server',  230, 190, { state: 'stopped', type: 't3.medium', vcpu: 2, ram: 4, ena: true }),
      svc('ebs',      'EBS gp3\n30GiB',     'db',      420, 260, { iops: 3000, throughput: 125, snapshots: 0 }),
      svc('eni',      'ENI (eth0)\nSG:x2',  'apigw',   420, 140, { sg: ['web-sg:80,443', 'ssh-sg:22'], publicIp: '54.123.45.67' }),
      svc('ami',      'AMI (amzn2-ami\nx86_64)', 'lambda', 620, 80),
      svc('asg',      'Auto Scaling\nGroup', 'server',  620, 200, { minSize: 1, maxSize: 5, desired: 2 }),
      svc('alb',      'ALB\n(Target Group)', 'apigw',   620, 320),
    ],
    edges: [
  ...
  s.nodes[1].state = 'running';
  s.metrics.instances = 1; s.metrics.cpu = 15;
  ...
  snap(steps, s, 'HIBERNATE: instance stops, RAM contents saved to EBS root volume (encrypted at rest). On start: RAM loaded back, processes resume where they left off — no OS boot, no app restart. Limits: only on-demand instances, max 150 GiB RAM, not all instance families, must have encrypted EBS. After hibernate: instance in STOPPED state — no compute charges, boot takes ~30s vs 2min for full cold boot.', 7);

  s.nodes[1].state = 'running';
  s.events.push({ type: 'ok', msg: 'Golden AMI: create custom AMI from configured instance (patched, apps installed). Launch new instances from AMI instantly.' });
  s.nodes[4].state = 'active';
  s.packets = [pkt('ec2', 'ami', 'CreateImage: ami-0golden', 'request')];
  ...
  snap(steps, s, 'ALB (Application Load Balancer): distributes incoming traffic across EC2 instances. Path-based routing: /api/* → backend, /* → frontend. Host-based routing: api.myapp.com → backend, app.myapp.com → frontend. Stickiness (session affinity): same client → same instance for session state. Connection draining: stops sending new requests to deregistering instance, waits for in-flight requests to complete. SG best practice: web-sg allows traffic from ALB SG only (not from 0.0.0.0/0).', 10);

  s.events.push({ type: 'ok', msg: 'Placement Group: cluster (low latency, 10 Gbps), spread (hardware isolation, max 7/group), partition (topology awareness, up to 7 partitions).' });
  snap(steps, s, 'Placement Groups: 1) CLUSTER — instances in same AZ, low latency (10 Gbps), good for HPC, tightly coupled apps. 2) SPREAD — each instance on separate hardware (max 7 instances per AZ), critical systems isolation. 3) PARTITION — instances divided into partitions (each partition on separate hardware), used for distributed systems (Hadoop, Cassandra). Spot Instances: up to 90% discount, AWS can reclaim (2min warning). Use for: batch processing, stateless web servers, machine learning training. Use Spot with ASG + mixed instances policy.', 11);
